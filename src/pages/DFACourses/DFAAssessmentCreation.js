...
import DFARegistrationLayout from 'Layout/DFALayout/DFARegistrationLayout';
import DFACreateAssessmentGradeForm from 'components/DFACourses/DFAAssesments/DFACreateAssessmentGradeForm';
import DFAUpsertQuestionFormList from 'components/DFACourses/DFAAssesments/DFAUpsertQuestionFormList';
      ...
      <>
        <Box style={{ display: isSingleEntry ? 'block' : 'none' }}>
          <Controller
            ...
            render={({ ...rest }) => (
              <DFAUpsertQuestionFormList {...rest} deleteQuestion={deleteQuestion} />
            )}
          />
        </Box>
          ...
          <Box style={{ borderBottom: `1px solid ${colors.secondaryLightGrey}` }}>
            ...
            >
              {Object.values(AssessmentEntry).map((tab) => (
                <StyledTab disableRipple label={tab} key={tab} />
              ))}
            </StyledTabs>
          </Box>
...
const StyledTabs = withStyles((theme) => ({
  ...
  <Tabs {...props} TabIndicatorProps={{ style: { background: '#3CAE5C' }, children: <span /> }} />
));

const StyledTab = styled((props) => <Tab style={{ color: '#3CAE5C' }} disableRipple {...props} />)(
  ({ theme }) => ({
    textTransform: 'none',
...
export default memo(DFAAssessmentCreation);
