...
const DFAAuthLayout = ({ children, imageSrc, title, description, renderSecondaryContent }) => {
  ...
  const renderTopSection = () => {
    return (
      <Box className={classes.header}>
        ...
        <Typography variant="h4" color="textPrimary" className="title">
          {title}
        </Typography>
        {description && (
...
const useStyles = makeStyles((theme) => ({
  ...
  header: {
    '& .title': {
      ...
      [theme.breakpoints.down('xs')]: {
        fontSize: '1.8rem', // Change the font size for xs screens
      },
    },
    '& .description': {
      paddingBottom: theme.spacing(15),
      [theme.breakpoints.down('xs')]: {
        fontSize: '1.1rem', // Change the font size for xs screens
      },
    },
  },
}));

DFAAuthLayout.propTypes = {
...
export default React.memo(DFAAuthLayout);
