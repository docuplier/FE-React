...
function GreenHeaderPageLayout(props) {
  const {
    tabs,
    onTabChange,
              ...
              <Grid container>
                <Grid item xs={12} lg={8} className={classes.leftContent}>
                  <div className="breadcrumb-wrapper">
                    <DFABreadcrumb
                      breadCrumbProps={{ className: classes.breadcrumb }}
                      links={links}
                    />
                  </div>
                  {!!avatarSrc && (
          ...
          <MaxWidthContainer>
            <StyledTabs value={tabIndex} onChange={handleChange}>
              {tabs?.map((tab, i) => (
                <Tab
                  disableRipple
                  label={<span style={{ color: i == tabIndex ? '#3CAE5C' : '' }}> {tab} </span>}
                  key={i}
                />
              ))}
            </StyledTabs>
...
GreenHeaderPageLayout.propTypes = {
...
export default GreenHeaderPageLayout;
