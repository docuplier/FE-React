...
const DFAUpsertQuestionForm = ({ data, onChange, onRemove }) => {
      ...
      >
        ...
        <Box ml={4}>
          <Radio
            ...
            value={optionId.id}
            classes={{
              root: classes.checked, // Apply the custom class to the root element of the Radio
            }}
            name={`option ${index}`}
  ...
  const renderMultiChoiceOptions = () => {
    return (
      <Box>
        ...
        <Box mt={8} display="flex" justifyContent="flex-end" color={'#3CAE5C'}>
          <Button color="inherit" onClick={handleAddOption} startIcon={<AddIcon />}>
            Add new option
          </Button>
        </Box>
      </Box>
...
export default memo(DFAUpsertQuestionForm);
