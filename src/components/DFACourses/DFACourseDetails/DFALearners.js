...
import { ReactComponent as AddIcon } from 'assets/svgs/plus-green.svg';
import { format } from 'date-fns';
import { Avatar, Box, Typography, Badge, Grid, Select } from '@material-ui/core';
...
import LearnerStatsModal from 'components/Courses/LearnerStatsModal';
...
import { useStyles } from '../../../reusables/DFALearnerCss';
    ...
    <Box className={classes.container}>
      <Box className={classes.container2}>
        <Typography className="text">
          Section to tell user that student should use their surname is small letter and the unique
          ID
        </Typography>
        <ul>
          <li>Lorem ipsum</li>
      ...
      <Grid container psacing={2}>
        ...
        <Grid item xs={3}>
          <Box className={classes.buttonContainer}>
            <LoadingButton className={classes.button} onClick={() => setOpen(true)}>
              <AddIcon /> &nbsp; Add Learners
            </LoadingButton>
          </Box>
        </Grid>
      </Grid>
      ...
      />
      <AddLearner open={open} onClose={() => setOpen(false)} />
    </Box>
  );
};

export default DFALearners;
