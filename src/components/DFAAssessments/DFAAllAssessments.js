...
export default function DFAAllAssessments({ data, handleChangeQueryParams, queryParams }) {
  ...
  const handleNavigationForLecturerAndSchoolAdmin = (assessmentId, status) => {
    if (status === AssessmentStatus.DRAFT) {
      return `${PrivatePaths.DFA_ASSESSMENTS}/create-assessment?id=${assessmentId}`;
    }

    return `${PrivatePaths.DFA_ASSESSMENTS}/${assessmentId}`;
  };
